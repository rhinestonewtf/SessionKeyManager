...
interface Interface {
    ...
    function getSessionData(address smartAccount, bytes32 sessionDigest)
        external
        view
        returns (SessionData memory data);
    function isModuleType(uint256 _type) external pure returns (bool);
    function isValidSignatureWithSender(address sender, bytes32 hash, bytes memory data)
        external
        view
        returns (bytes4);
    ...
    function validateUserOp(PackedUserOperation memory userOp, bytes32 userOpHash)
        external
